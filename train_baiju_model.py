...
class BaijuTrainer:
    ...
    def generate_text_new_huigui(self, prompt, max_length, temperature=1.0, top_k=20):
        ...
        with torch.no_grad():
            ...
            for _ in range(max_length):
                ...
                ctx_emb = self.embedding(ctx_tensor)
                if h is None:
                    output, h = self.model.step_generate(ctx_emb)
                else:
                    output, h = self.model.step_generate(ctx_emb, h)
    ...
    def train_epoch_teacher_forcing(self):
        ...
        for batch_idx, batch in enumerate(self.train_loader):
            ...
            outputs = self.model(embeddings)
            logits = self.output_layer(outputs)
            ce_loss = self.criterion(logits.view(-1, self.vocab_size), response.view(-1))
            loss = ce_loss
            ...
            if batch_idx % (self.config['log_interval'] * 100) == 0 and batch_idx > 0:
                print("\n测试文本生成...")
                prompt1 = "人工智能"
                generated1 = self.generate_text_new_huigui(prompt1, max_length=10, temperature=0.6, top_k=40)
                generated2 = self.generate_text_new_huigui(prompt1, max_length=10, temperature=0.8, top_k=40)
                generated3 = self.generate_text_new_huigui(prompt1, max_length=10, temperature=1.0, top_k=40)
                print(f"提示: {prompt1}\n生成: {generated1}")
                print(f"提示: {prompt1}\n生成: {generated2}")
                print(f"提示: {prompt1}\n生成: {generated3}")
...
def main():
    ...
    print("\n测试文本生成...")
    prompt = "人工智能"
    generated = trainer.generate_text_new_huigui(prompt, max_length=50, temperature=0.4,top_k=40)
    print(f"提示: {prompt}")

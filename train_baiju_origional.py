...
tokenizer = AutoTokenizer.from_pretrained("baichuan-inc/Baichuan-7B", trust_remote_code=True, use_fast=False)
print('加载完成')
TOKENIZER = tokenizer
import math
import time
...
class CompressedEmbedding(nn.Module):
    def __init__(self, vocab_size=150000, hidden_size=128, compressed_dim=32):
        super().__init__()
        self.embedding_low = nn.Embedding(vocab_size, compressed_dim)  # 150k * 32 = 4.8M
        self.proj_up = nn.Linear(compressed_dim, hidden_size, bias=False)  # 32 * 128 = 4k

    def forward(self, x):
        return self.proj_up(self.embedding_low(x))
class BaijuTrainer:
    def __init__(self, config):
        ...
        val_ratio = config.get('val_ratio', 1e-6)
        ...
        self.model = LM(
            ...
            cengshu=config['cengshu'],
            order=config['order'],
            dx=config['dx']
        ).to(self.device)
        #self.embedding = CompressedEmbedding(self.vocab_size, self.config['d_model'],self.config['compress_dim']).to(self.device)
        self.embedding = nn.Embedding(self.vocab_size, config['d_model']).to(self.device)
        self.embedding = self.embedding.cpu()
        self.embedding.weight = Int8Params(self.embedding.weight.data, requires_grad=True)
        ...
        #self.optimizer = torch.optim.NAdam(list(self.model.parameters()) + list(self.embedding.parameters()) + list(self.output_layer.parameters()),lr=5e-4,decoupled_weight_decay=True)
        #self.optimizer = torch.optim.SGD(list(self.model.parameters()) + list(self.embedding.parameters()) + list(self.output_layer.parameters()),lr=1e-3,momentum=0.9,nesterov=True)
        #self.optimizer = AnyPrecisionSophiaG(list(self.model.parameters()) + list(self.embedding.parameters()) + list(self.output_layer.parameters()),lr=config.get('learning_rate'),rho=config.get('rho'),use_kahan_summation=config.get('use_kahan_summation'))
        #self.scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(self.optimizer,factor=0.95,min_lr=1e-7)
        #self.scheduler = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(self.optimizer,T_0=3606399, T_mult=2, eta_min=1e-5)
    ...
    def generate_text_new(self, prompt, max_length, temperature=1.0, top_k=20, top_p=None, ):
        ...
        with torch.no_grad():
            ...
            ctx_tensor = torch.tensor([ctx], dtype=torch.long).to(self.device)
            ctx_emb = self.embedding(ctx_tensor)
            output = self.model.step_train(ctx_emb, max_length)
            logits = self.output_layer(output)[0] / temperature
            probs = torch.softmax(logits, dim=-1)
        ...
        self.model.train()
        return self.tokenizer.decode(generated_ids, skip_special_tokens=True)
    def generate_text_new_huigui(self, prompt, max_length, temperature=1.0, top_k=20):
        self.model.eval()
        input_ids = self.tokenizer.encode(prompt, add_special_tokens=False)
        generated_ids = input_ids.copy()
        ...
        with torch.no_grad():
            h = None
            for _ in range(max_length):
                ctx_tensor = torch.tensor([input[-self.config['seq_len']:]], dtype=torch.long).to(self.device)
                ctx_emb = self.embedding(ctx_tensor)
                if h is None:
                    output, h = self.model.step_generate(ctx_emb[:,-1,:], h)
                else:
                    output, h = self.model.step_generate(ctx_emb[:,-1,:], h)
                logits = self.output_layer(output)[0,0, :] / temperature
                if top_k:
    ...
    def validate_teacher_forcing(self):
        ...
        with torch.no_grad():
            for batch in self.val_loader:
                ...
                labels = batch['response'].to(self.device)
                input_ids = input_ids[:,:-1]
                labels = labels[:,1:]
                embeddings = self.embedding(input_ids)
                outputs,_ = self.model(embeddings,init=None)
                logits = self.output_layer(outputs)
                loss = self.criterion(logits.reshape(-1, self.vocab_size), labels.reshape(-1))
    ...
    def train_epoch_teacher_forcing(self):
        ...
        for batch_idx, batch in enumerate(self.train_loader):
            input_ids = batch['input_ids'].to(self.device)  # (B, L)
            response = batch['response'].to(self.device)  # (B, L)
            input_ids = input_ids[:, :-1]
            response = response[:, 1:]
            embeddings = self.embedding(input_ids)
            outputs,_ = self.model(embeddings,init=None)
            logits = self.output_layer(outputs)
            ce_loss = self.criterion(logits.reshape(-1, self.vocab_size), response.reshape(-1))
            loss = ce_loss
            ...
            if batch_idx % (self.config['log_interval']) == 0:
                ...
                print(f'[TF] Epoch: {self.current_epoch} | Batch: {batch_idx}/{len(self.train_loader)} | '
                      f'Loss: {avg_loss:.4f} | '
                      f'PPL: {ppl:.2f} ｜ ')
            if batch_idx % (self.config['log_interval'] * 100) == 0:
                avg_loss = total_loss / total_tokens
                validate_loss, validate_ppl = self.validate_teacher_forcing()
                chaju = avg_loss - validate_loss
                print(f'[TF] Epoch: {self.current_epoch} | Batch: {batch_idx}/{len(self.train_loader)} | '
                      f'valid_Loss: {validate_loss:.4f} | '
                      f'valid_PPL: {validate_ppl:.2f} ｜ '
                      f'chaju: {chaju:.4f}')
            if batch_idx % (self.config['log_interval'] * 5000) == 0:
                avg_loss = total_loss / total_tokens
                self.save_checkpoint(epoch=0,loss=avg_loss)
            if batch_idx % (self.config['log_interval'] * 100) == 0 and batch_idx > 0:
                print("\n测试文本生成...")
                prompt1 = "人工智能"
                generated1 = self.generate_text_new_huigui(prompt1, max_length=100, temperature=0.6, top_k=40)
                generated2 = self.generate_text_new_huigui(prompt1, max_length=100, temperature=0.8, top_k=40)
                generated3 = self.generate_text_new_huigui(prompt1, max_length=100, temperature=1.0, top_k=40)
                print(f"提示: {prompt1}\n生成: {generated1}")
                print(f"提示: {prompt1}\n生成: {generated2}")
                print(f"提示: {prompt1}\n生成: {generated3}")
        avg_loss = total_loss / total_tokens
    ...
    def train(self):
        ...
        try:
            ...
            print("\n训练被中断，保存当前状态...")
            self.save_checkpoint(current_epoch, avg_loss)
        finally:
            self.plot_losses()
...
def main():
    ...
    print("\n测试文本生成...")
    prompt = "人工智能"
    generated = trainer.generate_text_new_huigui(prompt, max_length=100, temperature=0.4,top_k=40)
    print(f"提示: {prompt}")
...
def train_continue():
    ...
    try:
        ...
        print("\n训练被中断，保存检查点...")
        trainer.save_checkpoint(trainer.best_loss)

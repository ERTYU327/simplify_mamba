...
class BaijuTrainer:
    ...
    def generate_text_new_huigui(self, prompt, max_length, temperature=1.0, top_k=20):
        ...
        with torch.no_grad():
            for _ in range(max_length):
                ...
                output = self.model.step_generate(ctx_tensor)
    ...
    def validate_teacher_forcing(self):
        ...
        with torch.no_grad():
            for batch in self.val_loader:
                ...
                loss = self.criterion(outputs.reshape(-1, self.vocab_size), labels.reshape(-1))
    ...
    def train_epoch_teacher_forcing(self):
        ...
        for batch_idx, batch in enumerate(self.train_loader):
            ...
            response = response[:, 1:]
            outputs = self.model(input_ids)
            ce_loss = self.criterion(outputs.reshape(-1, self.vocab_size), response.reshape(-1))
            loss = ce_loss
